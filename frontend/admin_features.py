...
async def signup_start(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.edit_text(
        CHOOSE_THE_DAY[0], reply_markup=await days_keyboard(language=0, prefix="appr")
    )
...
async def pair_chosen(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    await callback.message.edit_text(
        CHOOSE_THE_GYM[0], reply_markup=await gyms_keyboard(0, "appr", gym_list)
    )
...
@router.callback_query(
    lambda callback: (
        callback.data.startswith("approve") or callback.data.startswith("apprgym_")
    )
)
async def approve_students_query(
    callback: types.CallbackQuery, state: FSMContext
) -> None:
    """
    Подтверждение присутствия студентов на паре
    """
    ...
    if callback.data.startswith("approve"):
        ...
        await callback.message.answer(
            f"{student.last_name} {student.first_name} отмечен!"
        )
    ...
    buttons.row(types.InlineKeyboardButton(text=BACK[0], callback_data="apprpair__"))
    await callback.message.edit_text(
        APPROVE_SIGNUPS[0], reply_markup=buttons.as_markup()
    )
    await callback.answer()
