        ...
        if message.from_user.id != int(SUPERUSER_ID):
            ...
            if not data:
                ...
                await message.answer(
                    LONG_TIME_NO_SEE[int(data.language)] % (data.first_name)
                )
        else:
...
async def profile(message: types.Message, state: FSMContext) -> None:
    ...
    if not user_data:
        ...
        if not coach:
            ...
            buttons.row(
                types.InlineKeyboardButton(
                    text=APPROVE_SIGNUPS[0], callback_data="apprchoose"
                )
            )
            buttons.row(
                types.InlineKeyboardButton(text=EXPORT_DB, callback_data="getdatabase")
            )
            await message.answer(
                text=COACH_MENU
                % (
                    coach.last_name,
                    coach.first_name,
                    (coach.patronymic if coach.patronymic else ""),
                ),
        ...
        buttons = InlineKeyboardBuilder()
        buttons.row(
            types.InlineKeyboardButton(text=SIGN_UP[language], callback_data="sign_up")
        )
        buttons.row(
            types.InlineKeyboardButton(
                text=EDIT_PROFILE[language], callback_data="edit_profile"
            )
        )
        await message.answer(
