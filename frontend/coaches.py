...
async def gyms_coaches_keyboard(picked_gyms) -> InlineKeyboardMarkup:
    ...
    for i in range(len(GYMS_ALLOWED)):
        if picked_gyms[i]:
            ...
            text = ('❌ ' ) + GYMS_ALLOWED[i]
        buttons.append([InlineKeyboardButton(text=text, callback_data='pickme_gym_'+str(i))])
    buttons.append([InlineKeyboardButton(text=FINISH_GYMS, callback_data='finish_gyms')])
...
async def reg_coach_command(message: Message, state: FSMContext):
    if message.from_user.id == int(SUPERUSER_ID) :
        ...
        # await first_name_coach(message, state)
...
async def patronymic_coach(message: types.Message, state: FSMContext) -> None:
    if (not re.match(r'^[А-Яа-яЁё]+(?:[- ]?[А-Яа-яЁё]+)*$', message.text)) and (message.text!=NO_PATRONYMIC):
        kb = [[types.KeyboardButton(text=NO_PATRONYMIC)]]
        keyboard = types.ReplyKeyboardMarkup(keyboard=kb)
        await message.answer(ERROR_PATRONYMIC_COACH, reply_markup=keyboard)
    else:
        await state.update_data(patronymic_coach = (None if message.text==NO_PATRONYMIC else message.text))
        gyms = [False for i in range(len(GYMS_ALLOWED))]
        kb = await gyms_coaches_keyboard(gyms)
        await message.answer(GYMS_QUESTION, reply_markup=ReplyKeyboardRemove(remove_keyboard=True))
...
@router.callback_query(lambda callback: callback.data.startswith('pickme_gym_'), Coach.gyms)
...
async def finish_coach_registration(callback: types.CallbackQuery, state: FSMContext) -> None:
    ...
    gyms = data['gyms']
    secret_token = ''.join(random.choices(string.ascii_letters, k=50))
